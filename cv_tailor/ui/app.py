"""
Main Application Window — root Tk window, navigation, language picker, home button.
Steps: 0=Welcome 1=Upload 2=JD Analysis 3=Mode 4=Review 5=Preview
"""
...
class CVTailorApp(_BaseWindow):
    ...
    def _build_header(self):
        ...
        tk.Frame(self.header, bg=COLORS["accent"], width=8).pack(
            side="left", fill="y")
        ...
        privacy_frame = tk.Frame(
            self.header, bg=COLORS["bg_surface"], padx=10, pady=4)
    ...
    def _build_lang_picker(self):
        ...
        picker.pack(side="right", padx=PAD["sm"])
        picker.bind("<<ComboboxSelected>>", self._on_language_change)

    def _build_stepper(self):
        self.stepper_outer = tk.Frame(
            self, bg=COLORS["bg_secondary"], height=56)
        self.stepper_outer.pack(fill="x")
        self.stepper_outer.pack_propagate(False)

        self.step_labels = {}
        inner = tk.Frame(self.stepper_outer, bg=COLORS["bg_secondary"])
        inner.pack(expand=True)

        steps = i18n.get_steps(self.locale)   # returns list of (num, label)
        for i, (num, label) in enumerate(steps):
            if i > 0:
                tk.Frame(inner, bg=COLORS["border"],
                         height=2, width=40).pack(side="left", pady=22)

            step_container = tk.Frame(inner, bg=COLORS["bg_secondary"])
            step_container.pack(side="left", padx=4)

            circle = tk.Label(
                step_container, text=num, width=3,
                font=FONTS["small_bold"],
                bg=COLORS["bg_surface"], fg=COLORS["text_muted"])
            circle.pack(side="left")

            lbl = tk.Label(
                step_container, text=label,
                font=FONTS["small_bold"],
    ...
    def _rebuild_stepper(self):
        for w in self.stepper_outer.winfo_children():
            w.destroy()
        self.step_labels = {}
        inner = tk.Frame(self.stepper_outer, bg=COLORS["bg_secondary"])
        inner.pack(expand=True)
        ...
        for i, (num, label) in enumerate(steps):
            if i > 0:
                tk.Frame(inner, bg=COLORS["border"],
                         height=2, width=32).pack(side="left", pady=22)
            sc = tk.Frame(inner, bg=COLORS["bg_secondary"])
            sc.pack(side="left", padx=3)
            circle = tk.Label(sc, text=num, width=3,
                               font=FONTS["small_bold"],
                               bg=COLORS["bg_surface"], fg=COLORS["text_muted"])
            circle.pack(side="left")
            lbl = tk.Label(sc, text=label,
                            font=FONTS["small_bold"],
                            bg=COLORS["bg_secondary"], fg=COLORS["text_muted"])
            lbl.pack(side="left", padx=(4, 0))
            self.step_labels[i + 1] = (circle, lbl)
    ...
    def _init_frames(self):
        ...
        from ui.jd_analysis_frame import JDAnalysisFrame
        ...
        self.frames = {
            ...
            2: JDAnalysisFrame(self.content_area, app=self),   # NEW
            3: ModeFrame(self.content_area, app=self),
            4: ReviewFrame(self.content_area, app=self),
            5: PreviewFrame(self.content_area, app=self),
        }
